...
FILE* dcd_open_append(char *dcd_filename);
FILE* dcd_open_read(char *dcd_filename);
...
FILE* dcd_open_append(char *dcd_filename){
	FILE* dcdfd = fopen(dcd_filename, "a");
	return dcdfd;
}
...
FILE* dcd_open_read(char *dcd_filename){
	FILE* dcdfd = fopen(dcd_filename, "r");
	return dcdfd;
}
...
int dcd_read_frame(FILE* dcd_file, int N,  float *X, float *Y, float *Z){
	...
	fread(&iin, 4, 1, dcd_file);
	if(iin == 48){
		fread(X, 48+8, 1, dcd_file);
	}
	//printf("%d\n", iin);
	fread(X, 4*N, 1, dcd_file);
	fread(&iin, 4, 1, dcd_file);
	/*if(iin != 4*N){
			printf("ERROR reading dcd\n");
			exit(0);
		}*/
	//printf("%d\n", iin);
	fread(&iin, 4, 1, dcd_file);
