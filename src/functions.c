...
Vector getFENEDerivative_1(Vector dist){
	...
	//if (modeStructure == CRYSTAL_MODE ){
	//	float ref = dist - funcParam.refDist; //for crystal
	//	force = kspring_cov * ref;
	//} else {
...
double getFENEEnergy(double dist){
	...
	//if (modeStructure == CRYSTAL_MODE ){
	//	float ref = dist - funcParam.refDist; //for crystal
	//	force = -0.5f * kspring_cov * ref * ref;
	//} else {
...
inline void copyVector(Vector* v1, const Vector v2){
	v1->x = v2.x;
	v1->y = v2.y;
	v1->z = v2.z;
	v1->value = v2.value;
...
inline double getVectorComponent(const Vector v, int num){
	if (num == 0)
		return v.x;
	if (num == 1)
		return v.y;
...
void fillContactLists(){
	printf("Filling contacts list...\n");
	bonds = (int*)calloc(sop.aminoCount * MAX_BONDS, sizeof(int));
	bondsCount = (int*)calloc(sop.aminoCount, sizeof(int));
	bondsR0 = (double*)calloc(sop.aminoCount * MAX_BONDS, sizeof(double));

	int i, j, k;
	for (k = 0; k < sop.bondCount; k++){
		i = sop.bonds[k].i;
		j = sop.bonds[k].j;
		bonds[i * MAX_BONDS + bondsCount[i]] = j;
		bonds[j * MAX_BONDS + bondsCount[j]] = i;
		bondsR0[i * MAX_BONDS + bondsCount[i]] = sop.bonds[k].r0;
		bondsR0[j * MAX_BONDS + bondsCount[j]] = sop.bonds[k].r0;
		bondsCount[i]++;
		bondsCount[j]++;
	}

	native = (int*)calloc(sop.aminoCount * MAX_NATIVE, sizeof(int));
	nativeCount = (int*)calloc(sop.aminoCount, sizeof(int));
	nativeR0 = (double*)calloc(sop.aminoCount * MAX_NATIVE, sizeof(double));
	nativeEh = (double*)calloc(sop.aminoCount * MAX_NATIVE, sizeof(double));

	for(k = 0; k < sop.nativeCount; k++){
		i = sop.natives[k].i;
		j = sop.natives[k].j;
		native[i * MAX_NATIVE + nativeCount[i]] = j;
		native[j * MAX_NATIVE + nativeCount[j]] = i;
		nativeR0[i * MAX_NATIVE + nativeCount[i]] = sop.natives[k].r0;
		nativeR0[j * MAX_NATIVE + nativeCount[j]] = sop.natives[k].r0;
		nativeEh[i * MAX_NATIVE + nativeCount[i]] = sop.natives[k].eh;
		nativeEh[j * MAX_NATIVE + nativeCount[j]] = sop.natives[k].eh;
		nativeCount[i] ++;
		nativeCount[j] ++;
		if(nativeCount[i] > MAX_NATIVE || nativeCount[j] > MAX_NATIVE){
			printf("ERROR: Maximum number of native contacts exceeded the limit of %d.\n", MAX_NATIVE);
			exit(0);
		}
	}

	pairs = (int*)calloc(sop.aminoCount * MAX_PAIRS, sizeof(int));
	pairsCount = (int*)calloc(sop.aminoCount, sizeof(int));

	 for(k = 0; k < sop.pairCount; k++){
		 i = sop.pairs[k].i;
		 j = sop.pairs[k].j;
		 pairs[i * MAX_PAIRS + pairsCount[i]] = j;
		 pairs[j * MAX_PAIRS + pairsCount[j]] = i;
		 pairsCount[i] ++;
		 pairsCount[j] ++;

		if(pairsCount[i] > MAX_PAIRS || pairsCount[j] > MAX_PAIRS){
			printf("ERROR: Maximum number of pairs (%d, %d) exceeded the limit of %d.\n", pairsCount[i], pairsCount[j], MAX_PAIRS);
			exit(0);
		}
	 ...
	 for (i = 0; i < sop.aminoCount; i++)
		 totalPairs+= pairsCount[i]/2.0;

	 printf("Done filling contact list.\n");
...
inline void addMatrix(double* m_fin, const double* m_init){
	int i;
	for (i = 0; i < 9; i++)
		m_fin[i] += m_init[i];
}
...
void calculatePairs(){
	...
	for (i = 0; i < sop.aminoCount; i++){
		...
		for (j = i+1; j < sop.aminoCount; j++){
			copyVector(&dist, DCDtoVector(pdbData.atoms[j].x - pdbData.atoms[i].x, pdbData.atoms[j].y - pdbData.atoms[i].y, pdbData.atoms[j].z - pdbData.atoms[i].z));
			if ( i != j && dist.value < pairs_cutoff){
				int add = 1;
				for(k = 0; k < bondsCount[i]; k++){
					if(j == bonds[i * MAX_BONDS + k]){
						add = 0;
					}
				}
				if(add == 1){
					for(k = 0; k < nativeCount[i]; k++){
						if(j == native[i * MAX_NATIVE + k]){
							add = 0;
						}
					}
				}
