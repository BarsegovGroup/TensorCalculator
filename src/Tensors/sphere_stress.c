...
void initSphereStress(){
	...
	if (printPDBOn) {
		...
		sphereScale = getDoubleParameter("sphereScaleFactor", 1.0, 0);
	}
...
inline void printPDBSphereStress(){
	...
	for (i = 0; i < pdbData.atomCount; i++){
		...
		if ( strcmp(pdbData.atoms[i].segment, "") == 0)
			pdbData.atoms[i].segment[0] = pdbData.atoms[i].chain;
		pdbData.atoms[i].occupancy = atomSphereStress[i].x * sphereScale;
		pdbData.atoms[i].beta = atomSphereStress[i].y * sphereScale;
		pdbData.atoms[i].charge = atomSphereStress[i].z * sphereScale;
	}
